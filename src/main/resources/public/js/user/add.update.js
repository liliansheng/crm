layui.use(['form', 'layer','formSelects'], function () {
     ...
     *  addOrUpdateUser:（也就是确定按钮的）lay-filter属性值
     */
    form.on('submit(addOrUpdateUser)',function (data){//function (data):监听拿到要提交的所有数据
        //提交数据的加载层
        ...
        //请求的地址
        var url = ctx + "/user/add";//(请求到添加操作)

        //判断id是否为空，如果为空，则为更新操作
        if ($("[name = 'id']").val()){
            url = ctx + "/user/update";
        }

        //data.field:从data中拿到所有数据(得到所有表单元素的值)
        ...
        $.post(url,formData,function (result){
            ...
            if (result.code == 200){
                ...
                //跳转到指定页面(注册页面执行注册成功后才跳转，登录后执行添加用户不跳转页面，注册与添加用户是同一个功能)
    ...
    var userId = $("[name = 'id']").val();
     formSelects.config("selectId",{//下拉框的id:selectId
         type:"post",//请求方式
         searchUrl:ctx + "/role/queryAllRoles?userId="+userId,//请求地址
         keyName: "roleName",//下拉框中的文本值，要与返回的数据中的key 对应一致（此下拉框接收数据的格式为键值对形式）
         keyVal: 'id',//要与返回的数据中的value值 对应一致
     },true)
